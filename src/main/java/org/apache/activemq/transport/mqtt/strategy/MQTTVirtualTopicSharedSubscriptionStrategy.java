 ...
 */
public class MQTTVirtualTopicSharedSubscriptionStrategy extends AbstractMQTTSubscriptionStrategy {
    ...
    private static final String VIRTUALTOPIC_SHARED_PREFIX = "$shared/";

    private static final Logger LOG = LoggerFactory.getLogger(MQTTVirtualTopicSharedSubscriptionStrategy.class);
    ...
    public byte onSubscribe(String topicName, QoS requestedQoS) throws MQTTProtocolException {
        ...
        if (topicName.startsWith(VIRTUALTOPIC_SHARED_PREFIX)) {
            String converted = VIRTUALTOPIC_CONSUMER_PREFIX + "shared" + "." +
            		VIRTUALTOPIC_PREFIX + convertMQTTToActiveMQ(topicName.substring(
            		VIRTUALTOPIC_SHARED_PREFIX.length(),topicName.length()));            
            destination = new ActiveMQQueue(converted);
            consumerInfo.setPrefetchSize(ActiveMQPrefetchPolicy.DEFAULT_QUEUE_PREFETCH);
        	
        } else if (!protocol.isCleanSession() && protocol.getClientId() != null && requestedQoS.ordinal() >= QoS.AT_LEAST_ONCE.ordinal()) {
